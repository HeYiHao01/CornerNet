...
import torch
from nets.training_utils import focal_loss, reg_l1_loss
...
def fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen,
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            if backbone == "resnet50":
                ...
                # c_loss = 100. * focal_loss(hm, batch_hms)
                # off_loss = 10. * reg_l1_loss(offset, batch_regs, batch_reg_masks)
    ...
    if local_rank == 0:
        ...
        if (epoch + 1) % save_period == 0 or epoch + 1 == Epoch:
            torch.save(model.state_dict(), os.path.join(loss_history.log_dir, 'ep%03d-loss%.3f-val_loss%.3f.pth' % (
            epoch + 1, total_loss / epoch_step, val_loss / epoch_step_val)))

        if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
            print('Save best val model to best_val_weights.pth')
            torch.save(model.state_dict(), os.path.join(loss_history.log_dir, "best_val_weights.pth"))

        if len(loss_history.losses) <= 1 or (total_loss / epoch_step) <= min(loss_history.losses):
            print('Save best train model to best_train_weights.pth')
            torch.save(model.state_dict(), os.path.join(loss_history.log_dir, "best_train_weights.pth"))
