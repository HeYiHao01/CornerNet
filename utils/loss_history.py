...
class EvalCallback():
    def __init__(self, net, backbone, input_shape, class_names, num_classes, val_lines, log_dir, cuda, \
                 map_out_path=".temp_map_out", max_boxes=100, confidence=0.4, nms=True, nms_iou=0.5,
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        with torch.no_grad():
            ...
            top_label = np.array(results[0][:, -1], dtype='int32')
        ...
        for i, c in list(enumerate(top_label)):
            ...
            f.write("%s %s %s %s\n" % (predicted_class, score[:6], str(int(center_x)), str(int(center_y))))
