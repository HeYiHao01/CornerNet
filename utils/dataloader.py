...
class CornernetDataset(Dataset):
    def __init__(self, annotation_lines, input_shape, num_classes, train):
        super(CornernetDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if len(box) != 0:
            boxes = np.array(box[:, :2], dtype=np.float32)
            boxes[:, [0]] = np.clip(boxes[:, [0]] / self.input_shape[1] * self.output_shape[1], 0,
                                       self.output_shape[1] - 1)
            boxes[:, [1]] = np.clip(boxes[:, [1]] / self.input_shape[0] * self.output_shape[0], 0,
        ...
        for i in range(len(box)):
            ...
            if h > 0 and w > 0:
                ...
                #   计算真实框所属的特征点
                # -------------------------------------------------#
                ct = np.array([(bbox[0] + bbox[0]) / 2, (bbox[1] + bbox[1]) / 2], dtype=np.float32)
...
def corner_dataset_collate(batch):
