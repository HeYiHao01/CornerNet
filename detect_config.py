...
class CornerNet(object):
    _defaults = {
        "classes_path": 'config/classes_path.txt',

        # "model_path": 'logs/saved/res50_v1_0/best_val_weights.pth',  # resnet50 [640, 640]
        # "backbone": 'resnet50',
        # "confidence": 0.4,
    ...
    def generate(self, onnx=False):
        #   载入模型与权值
        assert self.backbone in ['resnet50', 'mobilenetv3']
        if self.backbone == "resnet50":
            self.net = CornerNet_Resnet50(num_classes=self.num_classes, pretrained=False)
        elif self.backbone == "mobilenetv3":
            self.net = CornerNet_MobilenetV3(num_classes=self.num_classes, pretrained=False)
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            predicted_keypoints.append([left, top])
            # area limit
            # h, w = image_shape
            # if 5 < left < w - 5 and 5 < top < h - 5:
            draw.ellipse([left, top, left + 6, top + 6], outline=self.colors[c], width=thickness+6)
            # draw.rectangle([left, top, left + 10, top + 10], outline=(0, 0, 255), width=thickness+6)
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            f.write("%s %s %s %s %s %s\n" % (
            predicted_class, score[:6], str(int(left)), str(int(top)), str(int(left+6)), str(int(top+6))))
